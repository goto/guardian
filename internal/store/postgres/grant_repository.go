...
func (r *GrantRepository) GenerateSummary(ctx context.Context, filter domain.ListGrantsFilter) (*domain.SummaryResult, error) {
	...
	dbGenWithLabels := func(gCtx context.Context, labels map[string][]string) (*gorm.DB, error) {
		// Override f.Labels so applyGrantsFilter only applies the given label filter,
	...
	if filter.SummaryLabelsV2 {
		sr.SummaryLabelsV2, err = generateLabelSummariesV2(ctx, dbGenWithLabels, "grants", `"_Appeal"."labels"`, filter.Labels)
		if err != nil {
			return nil, err
		}
	}
