...
type appealValidatorWithProvider interface {
	ValidateAppeal(ctx context.Context, a *domain.Appeal, p *domain.Provider) error
}
...
// Find records
func (s *Service) Find(ctx context.Context, filter domain.ListProvidersFilter) ([]*domain.Provider, error) {
	providers, err := s.repository.Find(ctx, filter)
	if err != nil {
		return nil, err
	}
...
func (s *Service) GetCount(ctx context.Context, filter domain.ListProvidersFilter) (int64, error) {
...
func (s *Service) FetchResources(ctx context.Context) error {
	providers, err := s.repository.Find(ctx, domain.ListProvidersFilter{})
	if err != nil {
		return err
	}
...
func (s *Service) ValidateAppeal(ctx context.Context, a *domain.Appeal, p *domain.Provider, policy *domain.Policy) error {
	...
	if validator, ok := c.(appealValidatorWithProvider); ok {
		if err := validator.ValidateAppeal(ctx, a, p); err != nil {
			return err
		}
	} else if validator, ok := c.(appealValidator); ok {
